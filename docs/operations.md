...
`atlas-trace-span-yard` is centered on observability work in Scala. The technical target is: Package a Scala local lab for span analysis with fixture event logs, golden state snapshots, and documented operating limits.
...
The score is deterministic. The current threshold is 181. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 3.
